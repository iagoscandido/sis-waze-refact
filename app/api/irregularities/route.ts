...
// Em: app/api/irregularities/route.ts

import { NextResponse } from "next/server";
import { getIrregularitiesLogic } from "@/lib/data/irregularities"; // Importe a nova função
...
export async function GET(req: Request) {
  try {
    ...
    // Chama a mesma lógica central com os parâmetros da URL
    const result = await getIrregularitiesLogic({ page, limit, city });

    return NextResponse.json(result);
  } catch (error: any) {
    console.error(error);
    // Trata o erro específico que criamos
